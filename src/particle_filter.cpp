...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	for(int i =0; i < num_particles; i++){
		...
		normal_distribution<double> dist_x(last_x, std_pos[0]);
		normal_distribution<double> dist_y(last_y, std_pos[1]);
		normal_distribution<double> dist_theta(last_theta, std_pos[2]);

		last_x = dist_x(gen);
		last_y = dist_y(gen);
		last_theta = dist_theta(gen);
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	...
	for(int i =0; i< observations.size();i++){
		LandmarkObs obs = observations[0];
		double last_distance = 0;
		for(int j =0; j< predicted.size();j++){
			double distance =sqrt( 
				pow(predicted[i].x - observations[j].x,2) 
				+ 
